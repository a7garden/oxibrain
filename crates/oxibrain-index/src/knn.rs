...
impl KnnIndex {
    ...
    pub fn insert(&mut self, id: String, v: TfIdfVector) {
        self.entries.push((id, v));
    }
    pub fn search(&self, q: &TfIdfVector, k: usize) -> Vec<(String, f64)> {
        let mut s: Vec<(String, f64)> = self
            .entries
            .iter()
            .map(|(id, v)| (id.clone(), cosine_sim(q, v)))
            .collect();
        s.sort_by(|a, b| {
            b.1.partial_cmp(&a.1)
