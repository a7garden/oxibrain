//! oxibrain-index: pure retrieval algorithms (RRF, TF-IDF, kNN, adjacency,
//! community label propagation). No rusqlite, no I/O — all algorithms are pure
//! functions over in-memory data structures.

#![cfg_attr(test, allow(clippy::unwrap_used))]
pub mod adjacency;
pub mod blocking;
pub mod community;
pub mod knn;
pub mod ngram;
pub mod quantize;
pub mod rrf;
pub mod spec;
pub mod vector;
pub use adjacency::{AdjacencyGraph, BfsResult, BfsSpec, WeightedAdjacencyGraph};
pub use blocking::{BlockingConfig, LshIndex, entropy_gate};
pub use community::{CommunityMap, label_propagation, label_propagation_weighted};
pub use knn::KnnIndex;
pub use ngram::{jaccard, lsh_bands, minhash, shingle_entropy, shingles};
pub use quantize::{cosine_approx, dequantize_i8, dequantize_signs, hamming, max_abs, quantize, quantize_i8};
pub use rrf::{FusedItem, fuse};
pub use spec::{Direction, PredicateFilter};
pub use vector::{TfIdfModel, TfIdfVector, cosine_sim, features};
