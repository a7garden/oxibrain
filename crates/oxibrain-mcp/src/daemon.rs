...
///
/// Periodically calls `brain.extract_uncached()` to drain the memory-plane
/// backlog (queue-less since v11). Stops when `stop` is signaled (e.g., by
/// the daemon shutdown path).
/// The single-writer discipline (P8) serializes the write-back; extraction
/// itself runs outside any transaction per §7.2.
pub async fn run_extraction_worker(
    brain: std::sync::Arc<Brain>,
    space: String,
    _config: oxibrain_core::extraction::ExtractorConfig,
    ...
    loop {
        tokio::select! {
            _ = ticker.tick() => {
                if let Err(e) = brain.extract_uncached(budget.max_episodes_per_batch).await {
                    tracing::warn!(
                        space = %space,
                        error = %e,
