//! Apple-platform implementation of the MLX adapter (see lib.rs).
...
impl LocalMxlLlm {
    ...
    pub fn load(spec: &str, n_ctx: usize) -> Result<Self, BrainError> {
        ...
        let dir = crate::weights::resolve_model_dir(spec).map_err(err)?;
...
fn load_inner(dir: &std::path::Path, n_ctx: i32) -> Result<Inner, String> {
    let cfg = crate::config::Qwen3Config::load(dir)?;
    let eos = crate::config::Qwen3Config::eos_token_ids(dir);
    let w = crate::weights::load_weights(dir)?;
